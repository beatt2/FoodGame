...
namespace Events
{
    public class Messages : MonoBehaviour
    {
        ...
        public void Add(string headline, float percentage)
        {
            ...
            if (Review)
            {
                go.GetComponent<MediaTypeNames.Image>().sprite = percentage < 0 ? EventManager.Instance.ReviewBackgroundNegative.GetRandom_Array() : EventManager.Instance.ReviewBackgroundPositive.GetRandom_Array();
            }
            else if (percentage < 0)
            {
               
                
                    go.GetComponent<MediaTypeNames.Image>().sprite = EventManager.Instance.MessageBackground[1];
